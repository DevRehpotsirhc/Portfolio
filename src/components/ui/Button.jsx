export const Button = ({ style = "", type = "button", disabled = false, onClick = null, inside = "", byside = "", insideStyle = "", bysideStyle = "", others = "", title = "" }) => {
    ...
    return (
        ...
        >
            <span
                className={`flex w-full font-semibold items-center justify-center border border-b-8 rounded-xl py-2 px-3 shadow-md group-hover/btn:shadow-medium-500/40 transition-all duration-200 backdrop-blur-md cursor-pointer
            ...
            {byside !== "" && (
                <p className={`pr-3 font-semibold group-hover/btn:underline underline-offset-4 ${bysideStyle}`}>
                    {byside}
                </p>
            )}
        </button>
